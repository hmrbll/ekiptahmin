...
Once a match's predictions are revealed but **before** a result is entered, each pick shows the most it could still earn — labelled **"en fazla N puan"**, optionally followed by **"(pen. dahil M)"** on knockout picks where a penalty shootout could pay more. After the result is entered this is replaced by the actual earned `GanyanScore.total`, shown on the round that earned it (see below).

The best case is split into a `ganyan.BestCase` pair (same split in the morning digest email):

- **`regulation`** (the headline "en fazla N") — the parimutuel payout the pick would earn **if the match ended exactly on its predicted scoreline**, counting the regulation (exact / diff / result) pools only: each pool is split among everyone whose own revealed pick would also win it under that hypothetical. So it differentiates picks the way real ganyan does — a lone scoreline shows the full pool, a popular one a thin slice.
- **`with_penalties`** (the parenthetical, shown only when it differs) — the best total once shootout pools are in reach. For a **draw-on-KO pick carrying a shootout** it's the full six-pool self-scenario (its scoreline hitting *implies* penalties). For a **decisive pick** the two scenarios are mutually exclusive — either the match ends as called (regulation pools) or it goes to penalties, where the pick loses every regulation pool (the effective score is level) but can still win the **penalty-winner** pool via its implied winner — so the parenthetical is the larger of the two. Without this path a decisive pick could *out-earn* its own displayed maximum on a shootout night (seen on AUS–EGY, R32). Group matches never show the parenthetical — they can't go to penalties.
...
The **pre-lock hint** applies the same rule to its count: on a knockout card it shows the **distinct number of players whose pick is on the real fixture** ("Gerçek eşleşmeyi N oyuncu tutturdu — tahminler gönderim süresi dolunca görünür", or "Gerçek eşleşmeyi henüz kimse tutturamadı — …") — exactly the players the card will list at reveal. The any-matchup predictor count would always read "everyone" in a knockout (each player filled their own bracket), which misled readers into thinking everyone had called the matchup. Group cards keep the plain "N oyuncu tahmin etti" count, since there the fixture is fixed and the two counts coincide.
